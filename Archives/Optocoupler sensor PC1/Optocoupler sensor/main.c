...
void Pinsetup(void)
{
	DDRC &= ~(1<<DDC1);// DDCC1 becomes an input pin
	PORTC |= (1<<PORTC1);//enables pull up resistor 
}
...
void interrupt_enabler(void)
{
	...
	EIMSK &= ~(1<<INT0);*/
	PCICR |= (1<<PCIE1);
	PCMSK1 |= (1<<PCINT9);// enables interupt at A1
	//PCMSK1 |= (1<<PCINT8);// enables interrupt at A0
